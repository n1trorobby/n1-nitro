...
AddEventHandler('nitro:check', function()
  ...
  local veh = GetVehiclePedIsIn(ped, false)
  local hash = GetEntityModel(veh)
  if GetEntitySpeed(veh)>0.1 then ESX.ShowNotification('~r~You need to stop the vehicle to install NOS'); return end
  if veh and IsThisModelACar(hash) then
    TriggerServerEvent('nitro:removeInventoryItem','nitro', 1)
  else
    ESX.ShowNotification('~o~You need to be in a vehicle to install NOS')
  end
end)
...
AddEventHandler('nitro:activated', function()
  ...
  exports["n1_taskbar"]:taskBar(10100,"Applying NOS")
  engineon = false
  Wait(3000)
  ...
  ESX.ShowNotification('~y~NOS Installed')
  engineon = true
  nitro = 100
end)
